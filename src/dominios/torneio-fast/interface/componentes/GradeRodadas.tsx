 ...
 */
interface PropsGradeRodadas extends PropsComAtualizacao {
  agora: number
}

export function GradeRodadas({ torneio, atualizarTorneio, agora }: PropsGradeRodadas) {
  ...
  const avancarAbas = () => {
    ...
    const primeiraAba = abas?.querySelector<HTMLElement>('.aba-grade')
    if (!abas || !primeiraAba) return

    const estilos = window.getComputedStyle(abas)
    const gap = Number.parseFloat(estilos.columnGap || estilos.gap) || 0
    abas.scrollBy({ left: primeiraAba.offsetWidth + gap, behavior: 'smooth' })
  }
  ...
  const mesasFiltradas = torneio.grade
    .flatMap((rodada, iRodada) =>
  ...
  return (
    <section aria-label="Grade de rodadas">
      <div className="cabecalho-grade-rodadas">
        <i className="fas fa-th-large icone-secao" aria-hidden="true" />
        <div className="titulo-grade-rodadas">
      ...
      <div className="navegacao-grade">
        <div ref={abasRef} className="abas-grade" aria-label="Filtrar grade por rodada">
          <button
        ...
        {torneio.grade.length > 2 && (
          <button
            className="indicador-mais-rodadas"
            type="button"
            aria-label="Mostrar próximas rodadas"
            onClick={avancarAbas}
          >
      ...
      <div className="grade-rodadas">
        {mesasFiltradas.map(({ rodada, iRodada, mesa, iMesa }) => {
          ...
          const segundosMesa = segundosRestantesMesa(torneio.timer, iRodada, iMesa, agora)
                ...
                <div className="resumo-tempo-mesa">
                  ...
                  {acrescimos > 0 && (
                    <small style={{ marginLeft: 'auto' }}>
                      +{acrescimos * (SEGUNDOS_ACRESCIMO_MESA / 60)} min
                    </small>
                  )}
                </div>
              ) : (
