...
import { carregarTorneio, salvarTorneio, criarTorneioVazio } from './logica/torneio/armazenamento'
import type { EstadoTorneio } from './logica/torneio/tipos'
import Cabecalho from './componentes/layout/Cabecalho'
import MenuInicial from './componentes/layout/MenuInicial'
import {
  ConfiguracaoTorneio,
  RankingGeral,
  TimerRodada,
  PainelQualidade,
  GradeRodadas,
} from './componentes/torneio/ComponentesTorneio'
 ...
 */
export type TelaPrincipal = 'inicio' | 'configuracaoTorneio' | 'calculadora' | 'suico' | 'referenciaYaku'

// ─── Helper ───────────────────────────────────────────────────────────────────

/** Retorna true se o torneio tem jogadores e grade montada. */
function torneioAtivo(t: EstadoTorneio): boolean {
  return t.jogadores.length > 0 && t.grade.length > 0
}
...
export default function App() {
   ...
   */
  const atualizarTorneio = useCallback((fn: (t: EstadoTorneio) => EstadoTorneio) => {
   ...
   */
  const aoIniciarTorneio = useCallback((t: EstadoTorneio) => {
    definirTorneio(t)
    setTela('inicio')
  }, [definirTorneio])
    ...
    <>
      ...
      */}
      <main className="conteudo-principal">
        ...
        {tela === 'calculadora' && (
          <PaginaCalculadora aoVoltar={() => setTela('inicio')} />
        )}
          ...
          <>
            ...
            <div style={{ position: 'fixed', bottom: 24, right: 24, zIndex: 100 }}>
              <button
                ...
                type="button"
                style={{ borderRadius: '50px', padding: '14px 20px', boxShadow: '0 4px 16px rgba(33,150,243,0.4)' }}
...
const CONFIG_PLACEHOLDER: Record<string, { icone: string; titulo: string; descricao: string }> = {
  suico: {
    ...
    descricao: 'Módulo de torneio suíço com múltiplas rodadas e emparelhamento dinâmico. Em desenvolvimento.',
  },
  referenciaYaku: {
    icone: 'fa-book',
    titulo: 'Referência de Yaku',
    descricao: 'Guia visual completo de todos os yaku com exemplos e valor em han. Em desenvolvimento.',
  },
}

function PlaceholderFuncionalidade({ tela, aoVoltar }: { tela: TelaPrincipal; aoVoltar: () => void }) {
