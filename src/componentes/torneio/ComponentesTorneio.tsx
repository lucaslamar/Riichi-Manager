...
import type { EstadoTorneio } from '../../logica/torneio/tipos'
import { parsearJogadores, validarJogadores } from '../../logica/torneio/jogadores'
import { embaralhar } from '../../logica/torneio/aleatorio'
import { gerarGrade } from '../../logica/pareamento/sorteio'
import { rotuloQualidade } from '../../logica/pareamento/qualidade'
import { criarTimerVazio } from '../../logica/torneio/armazenamento'
import {
...
} from '../../logica/torneio/chaves'
import { OPCOES_TIMER, PONTUACAO_INICIAL, SEGUNDOS_ACRESCIMO_MESA } from '../../logica/torneio/constantes'
...
export function ConfiguracaoTorneio({ aoIniciar, aoVoltar }: PropsConfiguracaoTorneio) {
  ...
  return (
    <section className="card" aria-label="Configurar torneio fast">
      ...
      {erro && (
        <div className="alerta-info" style={{ borderLeftColor: '#ef5350', background: '#fff5f5', color: '#b71c1c', marginBottom: 16 }}>
  ...
  return (
    <section className="card" aria-label="Classificação geral">
      <div className="cabecalho-secao">
        ...
        <button className="btn-contorno" type="button" onClick={aoExportarPdf} style={{ marginLeft: 'auto' }}>
...
export function TimerRodada({ torneio, atualizarTorneio }: PropsComAtualizacao) {
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      setTick((t) => t + 1) // força re-render para atualizar o display
  ...
  const temOpcaoPadrao = OPCOES_TIMER.some((o) => o.segundos === timer.totalSegundos)

  return (
    <section className="card card-timer" aria-label="Timer da rodada">
      <div className="cabecalho-timer">
        <div>
          <span style={{ display: 'block', color: '#738295', fontSize: '0.78rem', fontWeight: 900, textTransform: 'uppercase' }}>
            Timer base
          </span>
      ...
      <div className="controles-timer">
        ...
        <label className="seletor-timer">
          ...
          <select
            ...
            onChange={(e) => atualizarTorneio((t) => selecionarRodadaTimer(t, Number(e.target.value)))}
          >
            {torneio.grade.map((rodada, i) => (
              <option key={i} value={i}>Rodada {rodada.id}</option>
            ))}
          </select>
        </label>
        ...
        <label className="seletor-timer">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              if (!isNaN(val)) atualizarTorneio((t) => alterarDuracaoTimer(t, val))
            }}
          >
            {OPCOES_TIMER.map((o) => (
              <option key={o.segundos} value={o.segundos}>{o.rotulo}</option>
            ))}
            {!temOpcaoPadrao && (
      ...
      <p className="ajuda-timer">
        O timer base vale para todas as mesas. Acréscimos individuais (+5 min)
        aparecem apenas no cartão da mesa correspondente.
      </p>
    </section>
  )
}
...
export function PainelQualidade({ torneio, atualizarTorneio }: PropsComAtualizacao) {
  ...
  const handleRefazer = () => {
    if (!window.confirm('Refazer o sorteio mantendo os mesmos nomes?\nIsso apaga resultados e acréscimos.')) return

    atualizarTorneio((t) => {
      const embaralhados = embaralhar(t.jogadores)
      const grade = gerarGrade(embaralhados)
      return {
        ...t,
        jogadores: embaralhados,
        ...
        mesasConcluidas: {},
        pontuacoesPorMesa: {},
        timer: criarTimerVazio(0, t.timer.totalSegundos),
      }
    })
  }

  const metricaOponente = qualidade.limiteRepetidosPorJogador === null
  ...
  return (
    <section className="card" aria-label="Qualidade da grade">
      ...
      <dl className="metricas">
        <div><dt>Leste exato</dt><dd>{qualidade.lesteExato ? 'Sim' : 'Não'}</dd></div>
        <div><dt>Max. encontros por par</dt><dd>{qualidade.maxRepeticioesPar}</dd></div>
        <div><dt>Pares 2×</dt><dd>{qualidade.quantidadeDuasVezes}/{qualidade.quantidadeIdealRepetidos}</dd></div>
        <div><dt>Adversários rep./jogador</dt><dd>{metricaOponente}</dd></div>
        <div><dt>Pares 3×+</dt><dd>{qualidade.quantidadeTresVezesMais}</dd></div>
        <div><dt>Mesas iguais</dt><dd>{qualidade.mesasCompletasRepetidas}</dd></div>
      </dl>
      ...
      {qualidade.paresRepetidos.length > 0 && (
        <div style={{ marginTop: 16, borderTop: '1px solid #f0f0f0', paddingTop: 16 }}>
          <h3 style={{ margin: '0 0 10px', fontSize: '0.9rem', fontWeight: 900, textTransform: 'uppercase' }}>
...
interface PropsGrade extends PropsComAtualizacao {
  definirTorneio: (t: EstadoTorneio) => void
}
 ...
 *
 * @param props - torneio, atualizarTorneio, definirTorneio.
 */
export function GradeRodadas({ torneio, atualizarTorneio, definirTorneio }: PropsGrade) {
  ...
  const getPontuacao = (r: number, m: number, a: number) =>
    pontuacoes[`${r}_${m}_${a}`] ?? formatarPontuacao(torneio.grade[r]?.mesas[m]?.assentos[a]?.pontuacao ?? PONTUACAO_INICIAL)

  const setPontuacao = (r: number, m: number, a: number, val: string) =>
    setPontuacoes((prev) => ({ ...prev, [`${r}_${m}_${a}`]: val }))
  ...
  const handleSalvar = (indiceRodada: number, indiceMesa: number) => {
    ...
    const resultados = mesa.assentos.map((assento, a) => ({
      jogador: assento.jogador,
      pontuacao: parsePontuacao(getPontuacao(indiceRodada, indiceMesa, a) || String(PONTUACAO_INICIAL)),
    }))

    const resumo = resultados.map((r) => `${r.jogador}: ${formatarPontuacao(r.pontuacao)}`).join('\n')
    if (!window.confirm(`Confirmar resultados da Mesa ${indiceMesa + 1}?\n${resumo}`)) return

    // Atualiza a classificação e marca a mesa como concluída.
    atualizarTorneio((t) => {
      const classificacao = { ...t.classificacao }
      ...
      return {
        ...t,
        classificacao,
        mesasConcluidas: { ...t.mesasConcluidas, [chave]: true },
        pontuacoesPorMesa: {
          ...t.pontuacoesPorMesa,
          [chave]: resultados.map((r) => formatarPontuacao(r.pontuacao)),
        },
      }
    })
  }
  ...
  return (
    <section aria-label="Grade de rodadas">
      <div style={{ marginTop: 30, marginBottom: 20, borderBottom: '2px solid #f0f0f0', paddingBottom: 12, display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          rodada.mesas.map((mesa, iMesa) => {
            ...
            return (
              ...
              >
                ...
                <div className={`resumo-tempo-mesa ${estaRodandoEstaRodada ? '' : 'resumo-tempo-mudo'}`}>
                  <span>{estaRodandoEstaRodada ? 'Tempo da mesa' : `Outra rodada`}</span>
                  <strong>{estaRodandoEstaRodada ? formatarDuracao(segundosMesa) : `Rodada ${rodada.id}`}</strong>
                  <small>
                ...
                <div className="lista-assentos">
                  {mesa.assentos.map((assento, iAssento) => {
                    const valorInput = pontuacoesSalvas?.[iAssento] ?? getPontuacao(iRodada, iMesa, iAssento)
                    const classeVento = assento.vento.toLowerCase().replace('leste', 'leste').replace('sul', 'sul').replace('oeste', 'oeste').replace('norte', 'norte')
                    return (
                      <div className="linha-jogador" key={iAssento}>
                        ...
                        <input
                          ...
                          onFocus={(e) => {
                            if (e.target.value === pontuacaoInicialFormatada()) e.target.value = ''
                          }}
                          onBlur={(e) => {
                            if (!e.target.value.trim()) setPontuacao(iRodada, iMesa, iAssento, pontuacaoInicialFormatada())
                          }}
                          onChange={(e) => setPontuacao(iRodada, iMesa, iAssento, e.target.value.replace(/[^-0-9.,]/g, ''))}
                        />
                      </div>
                    )
                  })}
                ...
                <div className="acoes-mesa">
                  <button
                    ...
                    onClick={() => atualizarTorneio((t) => adicionarCincoMinutosMesa(t, iRodada, iMesa))}
                  >
                    <i className="fas fa-clock" />
                    {' '}+{SEGUNDOS_ACRESCIMO_MESA / 60} min mesa
